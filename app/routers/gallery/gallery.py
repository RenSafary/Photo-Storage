...
async def files(request: Request):
    files = get_files()
    return tmpl.TemplateResponse('gallery.html', {'request': request, 'files': files})
...
async def new_folder(
    ...
    try:
        while True:
            ...
            if Folders.get_or_none((Folders.user == user) & (Folders.name == new_folder)):
                await websocket.send_json(
                    {
                        "status":"error",
                        "details":"Folder name is already taken"
                    }
                )
                #return
            else: 
